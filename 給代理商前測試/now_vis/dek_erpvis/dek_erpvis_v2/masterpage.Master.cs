...
namespace dek_erpvis_v2
{
    public partial class masterpage : System.Web.UI.MasterPage
    {
        ...
        //public string color = "#8d9be3";//初始項目(小)顏色設定顏色修改相關(#8d9be3紫色)
        //public string button_color = "#4655cc";//側欄底下四個按鈕顏色修改相關(#2A3F54藍色)
        public string color = "";//初始項目(小)顏色設定顏色修改相關
        public string button_color = "#2A3F54";//側欄底下四個按鈕顏色修改相關
        string user_css = "";
        public string user_name = "";
        public string keys = "";
        string acc = "";
        public string give_js = "";
        public string[] ManageShowInit = new string[11] { "", "", "", "", "", "", "", "", "", "","" };
        public string OPManual = "";
        public string PSet = "";
        ...
        protected void Button_logout_Click(object sender, EventArgs e)
        {
            ...
            if (userInfo != null)
            {
                ...
                users_login_record(acc, 0);//0519-juiedit 登出確實要入log
                //Session.Abandon(); 執行global 的 seeeion end()
            }
        ...
        private void show_all()
        {
            ...
            //mysql
            //sql_cmd = "SELECT DISTINCT DEPARTMENT.DPM_ID,web_dpm, DEPARTMENT.DPM_NAME2,DEPARTMENT.DPM_NAME FROM   WEB_PAGES LEFT JOIN DEPARTMENT ON DEPARTMENT.DPM_NAME= WEB_PAGES.WEB_DPM WHERE length(DEPARTMENT.DPM_GROUP)=0 OR DEPARTMENT.DPM_GROUP IS NULL ";
                    ...
                    if (HtmlUtil.Check_DataTable(dt))
                    {
                        ...
                        if (HtmlUtil.Check_DataTable(dt))
                        {
                            ...
                            if (HtmlUtil.Check_DataTable(dt))
                            {
                                foreach (DataRow row in dt.Rows)
                                {
                                    string allpart = $"<li id=\"{"id" + i}\"><a id=\"{"Receive" + i}\"><img src=\"../../assets/images/{ DataTableUtils.toString(row["DPM_NAME"])}.png\" width=\"26px\" height=\"26px\"><label> &ensp; {DataTableUtils.toString(row["DPM_NAME2"])} </label><span class={"\"fa fa-chevron-down\""} style=\"font-size:10px\"></span></a>";
                                    PlaceHolder_AllPart.Controls.Add(new LiteralControl(allpart));
                                    html_js += js_code(i);
                                    i++;
                                }
                            }
                    ...
                    else
                    {
                        ...
                        if (HtmlUtil.Check_DataTable(dt))
                        {
                            ...
                            if (HtmlUtil.Check_DataTable(dt))
                            {
                                foreach (DataRow row in dt.Rows)
                                {
                                    string allitem = $"<li id=\"{"id" + i}\"><a id=\"{"Receive" + i}\"><img src=\"../../assets/images/{ DataTableUtils.toString(row["DPM_NAME"])}.png\" width=\"26px\" height=\"26px\"><label> &ensp; {DataTableUtils.toString(row["DPM_NAME2"])} </label><span class={"\"fa fa-chevron-down\""} style=\"font-size:10px\"></span></a>";

                                    PlaceHolder_AllPart.Controls.Add(new LiteralControl(allitem));
                                    html_js += js_code(i);
                                    i++;
                                }
                            }
        ...
        private void show_Manage()
        {
            //系統管理 頁面管控 看不看的見能操作是另外一個權限(為了符合不同公司需求)-05/12 jui
            //這版先不管key "數量"對就可以控制(想好方法再來控制key的正確性)
            //手冊
            manage_Showset("manage", "OPManualTitle", "OPManual");
            //參數
            manage_Showset("manage", "ParameterSetTitle", "PSetTag");
            //系統管理
            manage_Showset("manage", "ManageTitle", "ManageTitle");
            manage_Showset("manage", "Manage", ManageShowInit);
        }
        private void manage_Showset<T>(string FileName, string Section, T Key)
        {
            string TypeStr = "";
            Dictionary<string, string> MagInf = new Dictionary<string, string>();
            MagInf = HtmlUtil.Get_Ini_Section(FileName, Section);
            for (int i = 0; i < MagInf.Count; i++)
            {
                ...
                if (item.Value != "Y")
                {
                    if (Key is string)
                    {
                        TypeStr = Convert.ToString(Key);
                        if (TypeStr.ToUpper().Contains("PSETTAG"))
                            PSet = "hide";
                        else if (TypeStr.ToUpper().Contains("MANAGETITLE"))
                            ManageTitle = "hide";
                        else
                            OPManual = "hide";
                    }
                    else
                        ManageShowInit[i] = "none";
                }
            }
        ...
        {
            return $"if (id{i}.className == 'active') " +
                    "{" +
                        $"document.getElementById(\"Receive{i}\").click();" +
                    "}\n";
        }
        //從資料庫抓值後放到前端-------------
