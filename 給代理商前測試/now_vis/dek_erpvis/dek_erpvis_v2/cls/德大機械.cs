...
namespace dek_erpvis_v2.cls
{
    public class 德大機械
    {
        ...
        public static string 德大專用月份(string acc)
        {
            ...
            //if (HtmlUtil.Check_DataTable(dt))
            //{
            //    if (DataTableUtils.toString(dt.Rows[0]["DATE_STR"]).PadLeft(2, '0') == "01" && DataTableUtils.toString(dt.Rows[0]["DATE_END"]).PadLeft(2, '0') == "31")
            //    {
            //        startDate = DateTime.Now.ToString("yyyyMM") + DataTableUtils.toString(dt.Rows[0]["DATE_STR"]).PadLeft(2, '0');
            //        if (DataTableUtils.toInt(last) >= DataTableUtils.toInt(DataTableUtils.toString(dt.Rows[0]["DATE_END"]).PadLeft(2, '0')))
            ...
            //    else
            //    {
            //        if (DataTableUtils.toInt(start) >= DataTableUtils.toInt(DataTableUtils.toString(dt.Rows[0]["DATE_END"]).PadLeft(2, '0')))
        ...
        public static string get_title_web_path(string dp)
        {
            ...
            {
                string sqlcmd = $"SELECT DPM_NAME,DPM_NAME2 FROM DEPARTMENT where DPM_NAME != 'SYS'  and DPM_NAME = '{dp}' order by DPM_NAME2 ";
        ...
        public class 業務部_訂單變更紀錄
        {
            ...
            public static string 客戶列表(string 日期起, string 日期訖, string TOP)
            {
                ...
                return $"select {TOP}  a.客戶名稱 as 客戶名稱,count(a.訂單號碼) as 訂單次數,sum(a.變更次數) as 變更次數 from ( select  fjw.CUSTNM2 as 客戶名稱,TRN_NO as 訂單號碼,count(DISTINCT  jsn) as 變更次數 from  CordSub_chg_history as cc left join FJWSQL.dbo.CUST as fjw on cc.cust_no = fjw.CUST_NO  WHERE  cc.UP_time BETWEEN Cast('{日期起}' AS DATETIME) AND Cast('{日期訖}' AS DATETIME)  group by  fjw.CUSTNM2 ,TRN_NO ) as a group by 客戶名稱 order by 變更次數 desc";
            }
        ...
        public class 業務部_成品庫存分析
        {
            ...
            {
                return $"SELECT ASSEMBLY_LINE.LINE_ID,ASSEMBLY_LINE.LINE_NAME,ASSEMBLY_GROUP.GROUP_NAME FROM ASSEMBLY_LINE inner join ASSEMBLY_LINE_GROUP on ASSEMBLY_LINE.LINE_ID = ASSEMBLY_LINE_GROUP.LINE_ID inner join ASSEMBLY_GROUP on ASSEMBLY_LINE_GROUP.GROUP_ID = ASSEMBLY_GROUP.GROUP_ID where (ASSEMBLY_LINE.LINE_STATUS = '1'  OR ASSEMBLY_LINE.LINE_STATUS = 'True') and ASSEMBLY_LINE.LINE_ID = '{產線代號}'";
            }
        ...
        public class 業務部_未生產分析
        {
            ...
            public static string 未生產列表內容_客戶未生產詳細_All(string 條件式)
            {
                ...
                //新版
                return $" SELECT distinct sw_db.客戶簡稱, sw_db.產線代號, count(sw_db.產線代號) AS 小計 FROM (SELECT sw_CUST.custnm2 AS 客戶簡稱, sw_A22_FAB.FAB_USER AS 產線代號, sw_MKORDSUB.LOT_NO AS 製造批號, sw_a22_fab.cord_no AS 訂單號碼, sw_CORD.CORD_NO AS 客戶訂單, sw_MKORDSUB.item_no AS 品號, sw_item.itemnm AS 品名規格, sw_cordsub.trn_date AS 訂單日期, sw_A22_FAB.STR_DATE AS 預計開工日 FROM SW.FJWSQL.dbo.A22_FAB AS sw_A22_FAB LEFT JOIN SW.FJWSQL.dbo.CORD AS sw_CORD ON sw_CORD.trn_no = sw_A22_FAB.cord_no LEFT JOIN SW.FJWSQL.dbo.CORDSUB AS sw_CORDSUB ON sw_CORDSUB.TRN_NO = sw_A22_FAB.CORD_no AND sw_CORDSUB.SN = sw_A22_FAB.CORD_SN LEFT JOIN SW.FJWSQL.dbo.CUST AS sw_CUST ON sw_CUST.CUST_NO = sw_CORD.CUST_NO LEFT JOIN SW.FJWSQL.dbo.MKORDSUB AS sw_MKORDSUB ON sw_MKORDSUB.CORD_NO = sw_CORDSUB.trn_no AND sw_MKORDSUB.CORD_SN = sw_CORDSUB.sn LEFT JOIN SW.FJWSQL.dbo.citem AS sw_citem ON sw_CORDSUB.item_no = sw_citem.item_no LEFT JOIN SW.FJWSQL.dbo.item_22 AS sw_item_22 ON sw_CORDSUB.item_no = sw_item_22.item_no LEFT JOIN SW.FJWSQL.dbo.ITEM AS sw_item ON sw_item.ITEM_NO = sw_item_22.item_no LEFT JOIN SW.FJWSQL.dbo.ITEMIOS AS sw_ITEMIOS ON sw_ITEMIOS.MKORD_NO = sw_MKORDSUB.TRN_NO AND sw_ITEMIOS.MKORD_SN = sw_MKORDSUB.SN WHERE sw_CORDSUB.SCLOSE !='結案' AND sw_item.CLASS='Z' {條件式} AND sw_A22_FAB.FAB_USER > 0 AND (SELECT MAX(SW_ITEMIO.TRN_DATE) FROM SW.FJWSQL.dbo.ITEMIOS AS SW_ITEMIOS LEFT JOIN SW.FJWSQL.dbo.ITEMIO AS SW_ITEMIO ON SW_ITEMIOS.IO=SW_ITEMIO.IO AND SW_ITEMIOS.TRN_NO=SW_ITEMIO.TRN_NO WHERE SW_ITEMIOS.IO='2' AND SW_ITEMIOS.MKORD_NO=sw_MKORDSUB.TRN_NO AND SW_ITEMIOS.MKORD_SN=sw_MKORDSUB.SN AND SW_ITEMIO.S_DESC< >'歸還' AND SW_ITEMIO.MK_TYPE='成品入庫') IS NULL  ) AS sw_db LEFT JOIN 工作站狀態資料表 AS vis_db ON sw_db.製造批號 = vis_db.排程編號 where 進度 <> 100 OR 進度 IS NULL  group by sw_db.客戶簡稱, sw_db.產線代號 order by sw_db.客戶簡稱";

            }
            public static string 取得德大產線群組()
            {
                return "SELECT distinct ASSEMBLY_GROUP.GROUP_NAME FROM ASSEMBLY_LINE inner join ASSEMBLY_LINE_GROUP on ASSEMBLY_LINE.LINE_ID = ASSEMBLY_LINE_GROUP.LINE_ID inner join ASSEMBLY_GROUP on ASSEMBLY_LINE_GROUP.GROUP_ID = ASSEMBLY_GROUP.GROUP_ID where ASSEMBLY_LINE.LINE_STATUS = '1' OR ASSEMBLY_LINE.LINE_STATUS = 'True'  ";
            }
            public static string 取得產線群組與代號()
            {
                return "SELECT DISTINCT ASSEMBLY_GROUP.GROUP_NAME,ASSEMBLY_LINE.LINE_ID FROM ASSEMBLY_LINE INNER JOIN ASSEMBLY_LINE_GROUP ON ASSEMBLY_LINE.LINE_ID = ASSEMBLY_LINE_GROUP.LINE_ID INNER JOIN ASSEMBLY_GROUP ON ASSEMBLY_LINE_GROUP.GROUP_ID = ASSEMBLY_GROUP.GROUP_ID WHERE ASSEMBLY_LINE.LINE_STATUS = '1' OR ASSEMBLY_LINE.LINE_STATUS = 'True' ";
            }
            public static string 取得德大產線群組_單一(string 群組名稱)
            {

                return $"SELECT ASSEMBLY_GROUP.GROUP_NAME, ASSEMBLY_LINE.LINE_ID FROM ASSEMBLY_LINE inner join ASSEMBLY_LINE_GROUP on ASSEMBLY_LINE.LINE_ID = ASSEMBLY_LINE_GROUP.LINE_ID inner join ASSEMBLY_GROUP on ASSEMBLY_LINE_GROUP.GROUP_ID = ASSEMBLY_GROUP.GROUP_ID where (ASSEMBLY_LINE.LINE_STATUS = '1' OR ASSEMBLY_LINE.LINE_STATUS = 'True'  )and ASSEMBLY_GROUP.GROUP_NAME = '{群組名稱}' ";
            }
        ...
        public class 控制台_權限管理
        {
            ...
            {
                return $"SELECT DPM_NAME,DPM_NAME2 FROM DEPARTMENT where DPM_NAME != '{欲排除部門}' and DPM_ID not like '%DPM%' order by DPM_NAME2";
            }
