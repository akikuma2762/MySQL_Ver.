<%@ Page Title="" Language="C#" MasterPageFile="~/masterpage.Master" AutoEventWireup="true" CodeBehind="Enter_ReportView.aspx.cs" Inherits="dek_erpvis_v2.pages.dp_CNC.Enter_ReportView" MaintainScrollPositionOnPostback="true" Async="true" %>


<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <title>進站報工總覽 | <%= WebUtils.GetAppSettings("Company_Name") %></title>
    <%=color %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
            ...
            <div class="modal-content">
                <div class="modal-body">
                    <div id="testmoda22" style="padding: 5px 20px;">
                        ...
                        <asp:TextBox ID="TextBox_orderno" runat="server" Style="display: none"></asp:TextBox>
                                    ...
                                    <table id="Table_now" class="table table-ts table-bordered dt-responsive nowrap " cellspacing="0" width="100%">
                                        <thead>
                                            <tr style="background-color: white">
                                                ...
                                                <th style="text-align: center">當下產量</th>
    ...
    <script>
        ...
        //產生表格的HTML碼
        create_tablecode_noshdrow('Enter_ReportView', '進站報工總覽', 'datatable-buttons', '<%=th%>', '<%=tr%>');
        //產生相對應的JScode
        ...
        function Set_Information_ori(group, machine_show_name, status, Number, order_number, machine_name, now_infromation, now_staffs, type, bad_information, ok_qty, ng_qty, is_collect, orderno) {
            document.getElementById("<%=TextBox_Good.ClientID%>").disabled = false;
            //寫入工單
            ...
            //存入狀態
            $('#<%=TextBox_orderno.ClientID%>').val('' + orderno + '');
            ...
            if (is_collect == '是') {
                document.getElementById("<%=TextBox_Good.ClientID%>").disabled = true;
                document.getElementById("label_text").innerHTML = '調整量';

            }
            else {
                document.getElementById("<%=TextBox_Good.ClientID%>").disabled = false;
                document.getElementById("label_text").innerHTML = '本次數量';

            }
        }

        //工單號碼 機台名稱 本筆工單資訊 現時工作人員(webservice方式:多筆)
        function Set_Information(group, machine_show_name, status, Number, order_number, machine_name, now_infromation, now_staffs, type, bad_information, ok_qty, ng_qty, is_collect, orderno, ok_qty_Sum, ng_qty_Sum) {
            alert(ok_qty_Sum);
            alert(ng_qty_Sum);
            document.getElementById("<%=TextBox_Good.ClientID%>").disabled = false;
            //寫入工單
            ...
            //設定不良品數->改全部  因為同品號同工藝分配的關係
            if (parseInt(ng_qty_Sum, 10) > 0) {
                $('#<%=TextBox_NoGood.ClientID%>').val('' + ng_qty_Sum + '');
                $('#<%=TextBox_badqty.ClientID%>').val('' + ng_qty_Sum + '');
            }
            else {
                $('#<%=TextBox_NoGood.ClientID%>').val('' + 0 + '');
                $('#<%=TextBox_badqty.ClientID%>').val('' + 0 + '');
            ...
            //設定良品數
            if (parseInt(ok_qty_Sum, 10) > 0)
                $('#<%=TextBox_Good.ClientID%>').val('' + ok_qty_Sum + '');
            else
                $('#<%=TextBox_Good.ClientID%>').val('' + 0 + '');
        ...
        $(document).ready(function () {
            ...
            // 清空搜尋內容
            // 0622 juiedit
            // var div_content = document.getElementById('datatable-buttons_filter');0622
            ...
            $('#machine').keyup(function () {
                table.draw();
                // $(".dataTables_filter").parent(".pull-left").css("margin-left", "-120px");0622
        ...
        function add_staff(tablename, cleartext) {
            ...
            //檢查相同人員 是否已入站 0601 juiedit
            var sameor = false;
            var Odtables = ($('#Table_person').DataTable().rows().data());
            for (var i = 0; i < Odtables.length; i++) {
                var staff_list = Odtables[i];
                for (j = 0; j < staff_list.length; j++) {
                    //alert(staff_list[j]);
                    if (staff_list[j] == ('<center>' + id + '</center>')) {
                        sameor = true;
                    }
                }
            }
            if (!sameor) {
                ...
                alert('此人員已存在，請重新輸入');
                $(cleartext).val('' + '');
                //$(document.getElementById('<%=TextBox_Person.ClientID %>')).val('' + '');
            }
        }
        ...
        function staff_Exit(order, machine, staff, number) {
            $.ajax({
                ...
                data: { order: order, machine: machine, staff: staff, product_number: number, type: '進站報工' },
                success: function (xml) {
                    $(xml).find("ROOT_PIE").each(function (i) {
                        if ($(xml).find("ROOT_PIE").length > 0) {
        ...
        function previous_result(tablename) {
            ...
            $.ajax({
                ...
                data: { machine: document.getElementById('<%=TextBox_Machine.ClientID%>').value, manu_id: document.getElementById('<%=TextBox_Order.ClientID%>').value, type_mode: '進站報工', good_qty: count, bad_qty: document.getElementById("<%=TextBox_NoGood.ClientID%>").value },
                    ...
                    $(xml).find("ROOT_PIE").each(function (i) {
                        if ($(xml).find("ROOT_PIE").length > 0) {
                            $(this).children().each(function (j) {
                                ...
                                now_data.push('<center>' + $(this).attr("當下產量").valueOf() + '</center>');
