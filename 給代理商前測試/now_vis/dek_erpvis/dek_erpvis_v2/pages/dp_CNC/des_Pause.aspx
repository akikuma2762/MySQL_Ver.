<%@ Page Title="" Language="C#" MasterPageFile="~/masterpage.Master" AutoEventWireup="true" CodeBehind="des_Pause.aspx.cs" Inherits="dek_erpvis_v2.pages.dp_CNC.des_Pause" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <title id="titlename">暫停原因分析 | <%= WebUtils.GetAppSettings("Company_Name") %></title>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="right_col" role="main">
        ...
        <div id="myTabContent" class="tab-content">
                                ...
                                <div class="dashboard_graph x_panel">
                                    <div class="x_content">
                                        ...
                                        <div class="col-md-12 col-sm-6 col-xs-12">
                                            ...
                                            <div class="col-md-8 col-sm-9 col-xs-8">
                                                <asp:DropDownList ID="dropdownlist_Z" runat="server" CssClass="btn btn-default dropdown-toggle" Width="100%">
                                                    <asp:ListItem Value="總暫停" Selected="True">總暫停</asp:ListItem>
                                                    <asp:ListItem Value="暫停">暫停</asp:ListItem>
                                                    <asp:ListItem Value="除外暫停">除外暫停</asp:ListItem>
                                                </asp:DropDownList>
                                            </div>
                                        </div>
                                        <div class="col-md-12 col-sm-6 col-xs-12">
                                            ...
                                            <div class="col-md-8 col-sm-9 col-xs-8">
                                                <asp:DropDownList ID="dropdownlist_X" runat="server" CssClass="btn btn-default dropdown-toggle" Width="100%">
                                                    <asp:ListItem Value="人員名稱" Selected="True">人員名稱</asp:ListItem>
                                                    <asp:ListItem Value="設備名稱">設備名稱</asp:ListItem>

                                                    <asp:ListItem Value="異常類型">異常類型</asp:ListItem>
                                                </asp:DropDownList>
                                            </div>
                                        </div>
                                        <div class="col-md-12 col-sm-6 col-xs-12">
                                            ...
                                            <div class="col-md-8 col-sm-9 col-xs-8">
                                                <asp:DropDownList ID="dropdownlist_Y" runat="server" CssClass="btn btn-default dropdown-toggle" Width="100%">
                                                    <asp:ListItem Value="數量" Selected="True">數量</asp:ListItem>
                                                    <asp:ListItem Value="時間">時間(分)</asp:ListItem>
                                                </asp:DropDownList>
                                            </div>
                                        </div>
            ...
            <div role="tabpanel" class="tab-pane fade" id="tab_content3" aria-labelledby="profile-tab2">
                <div id="des_Pause_chart"></div>
            </div>
            <div role="tabpanel" class="tab-pane fade" id="tab_content2" aria-labelledby="profile-tab">
                <div id="des_Pause"></div>
            </div>
        </div>
    </div>
    ...
    <script>
        create_imgcode('des_Pause_img', 'export_image', 'chartContainer')
        //產生圖片
        set_column('chartContainer', '<%=dropdownlist_X.SelectedItem.Text.Replace("名稱","")%>暫停原因統計圖', '<%=subtitle%>', '<%=dropdownlist_X.SelectedItem.Text %>', '<%=dropdownlist_Y.SelectedItem.Text %>', '<%=dropdownlist_Y.SelectedItem.Text %>', [<%=col_data_Points%>]);
        //產生表格的HTML碼
        create_tablecode('des_Pause', '<%=dropdownlist_X.SelectedItem.Text.Replace("名稱","")%>暫停原因明細表', 'datatable-buttons', '<%=th.ToString() %>', '<%=tr.ToString() %>');
        //產生相對應的JScode
        ...
        //產生表格的HTML碼
        create_tablecode('des_Pause_chart', '<%=dropdownlist_X.SelectedItem.Text.Replace("名稱","")%>暫停原因統計表', 'datatable-buttonss', '<%=th_chart.ToString() %>', '<%=tr_chart.ToString() %>');
        //產生相對應的JScode
        ...
        $(document).ready(function () {
            ...
            if (urlname.indexOf('tab_content2') != -1 || urlname.indexOf('tab_content3') != -1)
                div_close('none');
            else
                div_close('block');
            //---------------------------------側欄收縮事件---------------------------------
        ...
        function div_close(ok, pages) {
            ...
            //輸出的檔案名稱是依據這個Title在變.....0518-juiedit
            if (pages == '1')
                title.innerHTML = '暫停原因分析 | <%= WebUtils.GetAppSettings("Company_Name") %>';
            else if (pages == '2')
                title.innerHTML = '<%=dropdownlist_X.SelectedItem.Text.Replace("名稱","")%>暫停原因統計表' + dateformat;
            else if (pages == '3')
                title.innerHTML = '<%=dropdownlist_X.SelectedItem.Text.Replace("名稱","")%>暫停原因明細表' + dateformat;
        }
