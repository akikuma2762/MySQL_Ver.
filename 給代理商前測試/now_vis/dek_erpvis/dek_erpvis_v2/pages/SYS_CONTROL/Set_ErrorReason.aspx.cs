...
{
    public partial class Set_ErrorReason : System.Web.UI.Page
    {
        int count = 1;
        public string acc = "";
        public string color = "";
        ...
        private void show_treeview()
        {
            ...
            string sql_cmd = "select * from 工作站結案異常類型資料表";
            ...
            if (HtmlUtil.Check_DataTable(dt))
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    P_Node.Text = $"{row["備註內容"]} ({row["異警持續顯示"]})";
                    P_Node.Value = $"{row["備註內容"]}-{row["異警持續顯示"]}";
                    P_Node.Checked = true;
        ...
        {
            GlobalVar.UseDB_setConnString(myclass.GetConnByDekdekVisAssmHor);
            string sqlcmd = "select * from 工作站結案異常類型資料表";
            ...
            if (dt != null)
            {
                ...
                try
                {
                    dt_Copy.Rows.Add(count, "異常", node.Text.Split(' ')[0], "", node.Value.Split('-')[1]);
                }
                catch
                {
                    dt_Copy.Rows.Add(count, "異常", node.Text.Split(' ')[0], "", node.Value);
                }
                count++;

                GlobalVar.UseDB_setConnString(myclass.GetConnByDekdekVisAssmHor);
                DataTableUtils.Insert_TableRows("工作站結案異常類型資料表", dt_Copy);
            }
        }
        ...
        private void Delete_Data()
        {
            //先清空該資料表
            GlobalVar.UseDB_setConnString(myclass.GetConnByDekdekVisAssmHor);
            bool ok = DataTableUtils.Delete_Record("工作站結案異常類型資料表", "備註編號 <> '0' ");
        }
        ...
        {
            string result = TreeViewOps.Add_Dpm(TreeView_Result, TextBox_dpm.Text, DropDownList_YN.SelectedItem.Value);
            if (result != "")
        ...
        protected void Button_Save_Click(object sender, EventArgs e)
        {
            ...
            foreach (TreeNode node in TreeView_Result.Nodes)
                CheckItems(node);
            Response.Write("<script>alert('設定完畢，即將進入設定畫面');location.href='Set_ErrorReason.aspx'; </script>");
        }
    }
}