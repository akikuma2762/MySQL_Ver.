...
{
    public partial class Asm_PersonNoFinish_Detail : System.Web.UI.Page
    {
        public string color = "";
        public string th = "";
        public string tr = "";
        ...
        private void GetCondi()
        {
            ...
            if (Request.QueryString["key"] != null)
            {
                ...
                Response.Redirect("Asm_PersonNoFinish.aspx", false);
        }
        ...
        private void Get_MonthTotal()
        {
            ...
            string sqlcmd = $"SELECT  工作站異常維護資料表.維護人員姓名, 工作站異常維護資料表.排程編號, 工作站型態資料表.工作站編號, 工作站型態資料表.工作站名稱 FROM 工作站異常維護資料表 LEFT JOIN (SELECT  父編號, 結案判定類型 FROM 工作站異常維護資料表 a WHERE a.結案判定類型 IS NOT NULL) a ON 工作站異常維護資料表.異常維護編號 = a.父編號 LEFT JOIN 工作站型態資料表 ON 工作站型態資料表.工作站編號 = 工作站異常維護資料表.工作站編號 left join 工作站狀態資料表 on 工作站狀態資料表.排程編號 = 工作站異常維護資料表.排程編號 WHERE (工作站異常維護資料表.父編號 IS NULL OR 工作站異常維護資料表.父編號 = 0) AND a.結案判定類型 IS NULL and (工作站狀態資料表.狀態 <> '完成' OR 進度 <> '100') and 工作站名稱 = '{workstation}' and 維護人員姓名 = '{staff}'  ";
        ...
        private void Set_Table()
        {
            ...
            tr = HtmlUtil.Set_Table_Content(true, Number, order_list, Asm_PersonNoFinish_Detail_callback).ToString();
        }

        private string Asm_PersonNoFinish_Detail_callback(DataRow row, string field_name)
        {
            string value = "";
            if (field_name == "維護人員姓名")
                value = staff;
            else if (field_name == "未結案數量")
            {
