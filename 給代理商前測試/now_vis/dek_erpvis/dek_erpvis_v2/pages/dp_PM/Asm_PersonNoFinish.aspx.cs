...
{
    public partial class Asm_PersonNoFinish : System.Web.UI.Page
    {
        public string color = "";
        ...
        private void Get_MonthTotal()
        {
            ...
            string sqlcmd = "SELECT  工作站異常維護資料表.維護人員姓名, 工作站異常維護資料表.排程編號, 工作站型態資料表.工作站編號, 工作站型態資料表.工作站名稱 FROM 工作站異常維護資料表 LEFT JOIN (SELECT  父編號, 結案判定類型 FROM 工作站異常維護資料表 a WHERE a.結案判定類型 IS NOT NULL) a ON 工作站異常維護資料表.異常維護編號 = a.父編號 LEFT JOIN 工作站型態資料表 ON 工作站型態資料表.工作站編號 = 工作站異常維護資料表.工作站編號 left join 工作站狀態資料表 on 工作站狀態資料表.排程編號 = 工作站異常維護資料表.排程編號 WHERE (工作站異常維護資料表.父編號 IS NULL OR 工作站異常維護資料表.父編號 = 0) AND a.結案判定類型 IS NULL and (工作站狀態資料表.狀態 <> '完成' OR 進度 <> '100')";
        ...
        private void Set_Table()
        {
            if (HtmlUtil.Check_DataTable(dt_MonthTotal))
            {
                ...
                if (HtmlUtil.Check_DataTable(staff))
                {
                    ...
                    tr = HtmlUtil.Set_Table_Content(true, staff, order_list, Asm_PersonNoFinish_callback);
                }
                else
        ...
        private string Asm_PersonNoFinish_callback(DataRow row, string field_name)
        {
            string value = "";
            if (linelist.IndexOf(field_name) != -1)
            {
                DataRow[] rows = dt_MonthTotal.Select($"維護人員姓名='{row["維護人員姓名"]}' and 工作站名稱='{field_name}'");
                string url = $"Asm_PersonNoFinish_Detail.aspx?key={WebUtils.UrlStringEncode($"staff={row["維護人員姓名"]},workstation={field_name}")}";
