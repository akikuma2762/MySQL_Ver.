...
namespace dek_erpvis_v2.pages.dp_APS
{
    public partial class APSList : System.Web.UI.Page
    {
        ...
        private void getdata(string start, string end, bool ok = false)
        {
            ...
            if (!ok)
            {
                ...
                if (HtmlUtil.Check_DataTable(dt))
                {
                    ...
                    check_status(dt, list, "OPEN", out Open_th, out Open_tr);
                    check_status(dt, list, "CLOSE", out Close_th, out Close_tr);
                    check_status(dt, list, "FAILURE", out Failure_th, out Failure_tr);
                }
                else
            ...
            else if (ok)
            {
                ...
                if (HtmlUtil.Check_DataTable(dt))
                {
                    ...
                    HtmlUtil.NoData(out th, out content);
            }
        }
        //callback執行的地方
        ...
        private void check_status(DataTable datatable, List<string> list, string status, out string th, out string tr)
        {
            th = "";
            tr = "";
