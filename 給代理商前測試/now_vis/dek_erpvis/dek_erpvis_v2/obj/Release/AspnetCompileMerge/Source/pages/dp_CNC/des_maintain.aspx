<%@ Page Title="" Language="C#" MasterPageFile="~/masterpage.Master" AutoEventWireup="true" CodeBehind="des_maintain.aspx.cs" Inherits="dek_erpvis_v2.pages.statistics.maintain" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <title id="titlename">生產資料分析 | <%= WebUtils.GetAppSettings("Company_Name") %></title>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="right_col" role="main">
        ...
        <div id="myTabContent" class="tab-content">
                                        ...
                                        <div class="col-md-12 col-sm-6 col-xs-12">
                                            ...
                                            <div class="col-md-8 col-sm-9 col-xs-8">
                                                <asp:DropDownList ID="dropdownlist_Y" runat="server" CssClass="btn btn-default dropdown-toggle" Width="100%">
                                                    ...
                                                    <asp:ListItem Value="工單">工單</asp:ListItem>
                                                </asp:DropDownList>
                                            </div>
                                        </div>
            ...
            <div role="tabpanel" class="tab-pane fade" id="tab_content3" aria-labelledby="profile-tab2">
                <div id="des_maintain_chart"></div>
            </div>

            <div role="tabpanel" class="tab-pane fade" id="tab_content2" aria-labelledby="profile-tab">
                <div id="des_maintain"></div>
            </div>
        </div>
    </div>
    ...
    <script>
        create_imgcode('des_maintain_img', 'export_image', 'chartContainer')
        //產生圖片
        set_column('chartContainer', '<%=dropdownlist_X.SelectedItem.Text.Replace("名稱","")%>生產統計圖', '<%=subtitle%>', '<%=dropdownlist_X.SelectedItem.Text %>', '<%=dropdownlist_Y.SelectedItem.Text %>', '<%=unit %>', [<%=col_data_Points%>]);


        //產生表格的HTML碼                            
        create_tablecode('des_maintain', '<%=dropdownlist_X.SelectedItem.Text.Replace("名稱","")%>生產<%=dropdownlist_Y.SelectedItem.Value.Replace("名稱","")%>明細表', 'datatable-buttons', '<%=th.ToString() %>', '<%=tr.ToString() %>');
        //產生相對應的JScode
        ...
        //產生表格的HTML碼
        create_tablecode('des_maintain_chart', '<%=dropdownlist_X.SelectedItem.Text.Replace("名稱","")%>生產<%=dropdownlist_Y.SelectedItem.Value.Replace("名稱","")%>統計表', 'datatable-buttonss', '<%=th_chart.ToString() %>', '<%=tr_chart.ToString() %>');
        //產生相對應的JScode
        ...
        function div_close(ok,pages) {
            ...
            if (pages == '1')
                title.innerHTML = '生產資料分析 | <%= WebUtils.GetAppSettings("Company_Name") %>';
            else if (pages == '2')
                title.innerHTML = '<%=dropdownlist_X.SelectedItem.Text.Replace("名稱","")%>生產<%=dropdownlist_Y.SelectedItem.Value.Replace("名稱","")%>統計表' + dateformat;
            else if (pages == '3')
                title.innerHTML = '<%=dropdownlist_X.SelectedItem.Text.Replace("名稱","")%>生產<%=dropdownlist_Y.SelectedItem.Value.Replace("名稱","")%>明細表' + dateformat;
