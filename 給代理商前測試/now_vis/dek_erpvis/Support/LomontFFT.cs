...
namespace Support
{
    public class LomontFFT
    {
        ...
        public void FFT(double[] complex_data, bool forward)
        {
            var n = complex_data.Length;
            ...
            Reverse(complex_data, n); // bit index data reversal
            ...
            while (n > mmax)
            {
                ...
                for (var m = 0; m < istep; m += 2)
                {
                    ...
                    {
                        var j = k + istep;
                        var tempr = wr * complex_data[j] - wi * complex_data[j + 1];
                        var tempi = wi * complex_data[j] + wr * complex_data[j + 1];
                        complex_data[j] = complex_data[k] - tempr;
                        complex_data[j + 1] = complex_data[k + 1] - tempi;
                        complex_data[k] = complex_data[k] + tempr;
                        complex_data[k + 1] = complex_data[k + 1] + tempi;
                    }
        ...
        /// <param name="forward">true for a forward transform, false for 
        /// inverse transform</param>
        public void FFT_ByTable(double[] complex_data, bool forward)
        {
            var n = complex_data.Length;
            // checks n is a power of 2 in 2's complement format
            if ((n & (n - 1)) != 0)
                throw new ArgumentException(
                    "complex_data length " + n + " in FFT is not a power of 2"
                    );
            n /= 2;    // n is the number of samples
        ...
        /// </param>
        /// <param name="forward">
        /// true for a forward transform, 
        /// false for inverse transform
        /// </param>
        public void FFT_Real(double[] complex_data, bool forward)
        {
            var n = complex_data.Length; // # of real inputs, 1/2 the complex length
            // checks n is a power of 2 in 2's complement format
            if ((n & (n - 1)) != 0)
                throw new ArgumentException(
                    "data length " + n + " in FFT is not a power of 2"
            ...
            {
                var k = n / 2 - j;
                var tkr = complex_data[2 * k];    // real and imaginary parts of t_k  = t_(n/2 - j)
                var tki = complex_data[2 * k + 1];
                var tjr = complex_data[2 * j];    // real and imaginary parts of t_j
                var tji = complex_data[2 * j + 1];
                ...
                // compute entry y[j]
                complex_data[2 * j] = 0.5 * (e + sign * (a + b));
                complex_data[2 * j + 1] = 0.5 * (f + sign * (d - c));

                // compute entry y[k]
                complex_data[2 * k] = 0.5 * (e - sign * (b + a));
                complex_data[2 * k + 1] = 0.5 * (sign * (d - c) - f);
            ...
            if (forward)
            {
                ...
                var temp = complex_data[0];
                complex_data[0] += complex_data[1];
                complex_data[1] = temp - complex_data[1];
            }
            else
            {
                var temp = complex_data[0]; // unpack the y0 and y_{N/2}, then invert FFT
                complex_data[0] = 0.5 * (temp + complex_data[1]);
                complex_data[1] = 0.5 * (temp - complex_data[1]);
                // do packed inverse (table based) FFT. This can be changed to regular inverse FFT to save memory
                FFT_ByTable(complex_data, false);
        ...
        public double[] ComplexArray(int data_size, int[] data)
        {
            //double:1,2,3,4,... --> complex:1,0,2,0,3,0,...
            int complex_size = NearestPowerOf2(data_size, true) * 2;
            double[] complex_data = new double[complex_size];
            for (int index = 0, pos = 0; index < complex_size;)
            {
                //real value part
                if (pos < data_size)
                    complex_data[index++] = data[pos++];
                else
        ...
        public double[] ComplexArray(int data_size, float[] data)
        {
            //double:1,2,3,4,... --> complex:1,0,2,0,3,0,...
            int complex_size = NearestPowerOf2(data_size, true) * 2;
            double[] complex_data = new double[complex_size];
        ...
        public double[] ComplexArray(int data_size, double[] data)
        {
            //double:1,2,3,4,... --> complex:1,0,2,0,3,0,...
            int complex_size = NearestPowerOf2(data_size, true) * 2;
            double[] complex_data = new double[complex_size];
        ...
        public int NearestPowerOf2(int value, bool is_less = false)
        {
            int number = value;
            number--;
            number |= number >> 1;
...
#region Test Data
            ...
            Action<int, int> testAction = (testIndex, paramIndex) =>
            {
                ...
                success &= Test(fftObject.FFT_Real, test, answerReal);
                success &= Test(fftObject.FFT, test, answerComplex);
                success &= Test(fftObject.FFT_ByTable, test, answerComplex);
            };
